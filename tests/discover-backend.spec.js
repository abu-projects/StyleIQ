...
test("Closet outfits and catalog products use their separate backend paths", async ({ page }) => {
  ...
  await catalog.getByRole("button", { name: /Save to Wishlist: Cotton Shirt/ }).click();
  await expect(catalog.getByRole("button", { name: /Remove from Wishlist: Cotton Shirt/ })).toBeVisible();
