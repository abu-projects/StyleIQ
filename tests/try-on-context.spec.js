...
test.describe('Try-On context entry points', () => {
  test('outfit family, Saved Look, Profile, and community entries preserve their source Look', async ({ page }) => {
    await readyTwin(page);
    for (const source of [
      { route: 'D-04', action: 'Try on', title: 'Coffee Meeting' },
      { route: 'G-02', action: 'Try On', title: 'Design Review' },
      { route: 'L-01', action: 'Style Twin', title: 'Design Review' },
      { route: 'K-04', action: 'Try on me', title: 'Coffee Meeting' },
    ]) {
