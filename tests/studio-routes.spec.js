...
test('F routes open their requested tools while retaining the same recreation draft',async({page})=>{
  ...
  await navigate('F-03');
  await expect(page.locator("#app").getByLabel('On My Twin preview',{exact:true})).toBeVisible();
  await navigate('F-02');
  await expect(page.locator("#app").getByLabel('Flat Lay preview',{exact:true})).toBeVisible();
  await navigate('F-04');
  await page.locator("#app").getByLabel('Location',{exact:true}).fill('Alexandria');
  await page.locator("#app").getByLabel('Location',{exact:true}).press('Tab');
  await page.reload();
  await expect(page.locator("#app").getByLabel('Location',{exact:true})).toHaveValue('Alexandria');
...
test('new Look starts visually and Lens returns a chosen Closet match to the same draft',async({page})=>{
  ...
  await page.locator('#app').getByRole('button',{name:'Edit Look details'}).click();
  await page.locator("#app").getByRole('button',{name:'New Look',exact:true}).click();
  await expect(page.locator("#app").getByLabel('Start your Look',{exact:true})).toBeVisible();
  await page.locator("#app").getByRole('button',{name:'Use Lens',exact:true}).click();
  await page.locator("#app").getByRole('dialog',{name:'StyleIQ Lens',exact:true}).getByRole('button',{name:'Photo library',exact:true}).click();
  await page.locator("#app").getByRole('button',{name:/Find owned alternatives/}).click();
...
test('advanced hide, reorder and remove retain the remaining canonical layers',async({page})=>{
  ...
  await page.locator("#app").getByRole('button',{name:'Hide Top',exact:true}).click();
  await expect(page.locator("#app").getByLabel('Flat Lay preview',{exact:true}).getByRole('button',{name:/Edit Top:/})).toHaveCount(0);
  await top.locator('summary').click();
  await page.locator("#app").getByRole('button',{name:'Show Top',exact:true}).click();
  await top.locator('summary').click();
  await page.locator("#app").getByRole('button',{name:'Move Top up',exact:true}).click();
  ...
  await page.locator("#app").getByRole('button',{name:'Remove Top',exact:true}).click();
...
test('render completion never replaces edits made while the renderer was pending',async({page})=>{
  ...
  await page.locator("#app").getByRole('button',{name:'Update Try-On',exact:true}).click();
  await expect(page.locator("#app").getByRole('button',{name:'Updating…',exact:true})).toBeDisabled();
  ...
  await page.locator("#app").getByRole('button',{name:'Update Try-On',exact:true}).click();
  await expect(page.locator("#app").getByLabel('On My Twin preview',{exact:true}).getByRole('img',{name:'Current Look on your Style Twin',exact:true})).toBeVisible();
  await page.reload();
  await expect(page.locator("#app").getByLabel('On My Twin preview',{exact:true}).getByRole('img',{name:'Current Look on your Style Twin',exact:true})).toBeVisible();
});
