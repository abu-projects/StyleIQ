...
test('existing Twin switches Looks without setup and returns to the selected Today context', async ({ page }) => {
  ...
  await page.locator("#app").getByRole('button', { name: 'Try another Look' }).click();
...
test('side arrows, drag and keyboard inspect the same full-body Look', async ({ page }) => {
  ...
  await page.locator("#app").getByRole('button', { name: 'Next view', exact: true }).click();
  await page.locator("#app").getByRole('button', { name: 'Next view', exact: true }).click();
...
test('E-06 is unique and direct entry shares the Today result and angle state', async ({ page }) => {
  ...
  await tryOn(page).click();
  await page.locator("#app").getByRole('button', { name: 'Previous view', exact: true }).click();
  ...
  await expect(page.locator("#app").getByRole('button', { name: 'Make it mine', exact: true })).toBeVisible();
  await page.locator("#app").getByRole('button', { name: 'Next view', exact: true }).click();
